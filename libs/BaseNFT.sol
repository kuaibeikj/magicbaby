...
contract BaseNFT0 is ERC721PresetMinterPauserAutoId {
    ...
    function mintBase(address _to, uint _val) public auth(GLOBAL_ROLE) {
        _mint(_to, createdTokenLength);
        IGraphSql(graphSqlAddress).transferNFT(address(this), address(0), _to, createdTokenLength, _val);
        createdTokenLength++;
    }

    function burnBase(uint256 _tokenId) public {
        burn(_tokenId);
        IGraphSql(graphSqlAddress).transferNFT(address(this), tx.origin, address(0), _tokenId, 0);
    }
